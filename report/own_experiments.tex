...
on complex sentences, containing one or more intervening
nouns of opposite number (attractors).
...
To establish number agreement,
the model must identify the noun that acts as the head of the subject.
This is challenging for sentence prefixes that contain multiple nouns.
Function words such as 'that' or 'of' carry 
important information about the syntactic structure of the sentence.
In this Section we investigate if the model
uses this information to establish number agreement
in the case of multiple nouns.
We limit our analysis to prefixes 
with exactly two nouns of opposite number.
...
picking the two nouns and the verb from a set of frequently
